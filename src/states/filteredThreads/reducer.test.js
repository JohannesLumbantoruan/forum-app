/**
 * test scenario for filteredThreadReducer
 *
 * - filteredThreadsReducer function
 *  - should return the initial state when given unknown action
 *  - should return the filteredThreads when given FILTER_THREADS action
 */

import { describe, expect, it } from 'vitest';
import filteredThreadsReducer from './reducer';

describe('filteredThreadsReducer function', () => {
    const initialState = [];
    ...
    it('should return the filteredThreads when given FILTER_THREADS action', () => {
        // arrange
        const threads = [
            {
        ...
        // assert
        expect(nextState).toEqual([threads[0]]);
    });
});